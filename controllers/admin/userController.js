    ...
    if (req.body.password === req.body.cpassword) {
        if (schema.validate(req.body.password)) {
            bcrypt.hash(req.body.password, 12, async (error, hash) => {
                if (error) {
                    ...
                    try {
                        ...
                        error.keyValue?.email ? errorMessage.email = "email Already Exist" : null
            ...
            res.render("admin/user/create", {
                title: "Admin- create User",
                errorMessage: {
                    password: "Invalid Password.it Must container at least 1 lower upper and digit"
                },
                data: data,
                session: req.session,
            })
    ...
    try {
        ...
        if (data) {
            ...
            mailer.sendMail({
                ...
                to: data.email,
                subject: `OTP Password Reset:Team BabyCare ${process.env.SITE_NAME}`,
                text: ` 
                    Hello ${data.name}
...
// forget password 2 OTP Enter page
...
// forget password reset password page
        ...
        if (data) {
            if (req.body.password === req.body.cpassword) {
                if (schema.validate(req.body.password)) {
                    ...
                    res.render("admin/user/forget-password3", {
                        title: "Admin-Reset Password",
                        errorMessage: "Invalid password need atleast 1 uppercase lowercase & digit"
                    })
                ...
                res.render("admin/user/forget-password3", {
                    title: "Admin-Reset Password",
                    errorMessage: "password & confirm password not matched"
                })
